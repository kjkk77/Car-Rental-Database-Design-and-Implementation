...
Compiler: Hongjoon Kim
*/

/* Introduction
The purpose of this script is to compile the query trials from 
the four team members including myself. The queries and their 
descriptions written by each member have not been changed.
...
/* Hongjoon's Queries */
 ...
 SELECT 
    V.VEHI_TYPE,
    TO_CHAR(SUM(M.MAIN_COST), '$999.99') AS 'TOTAL MAINTENANCE COST'
FROM
    VEHICLE V
        RIGHT JOIN
...
-- Lists vehicle types with the total maintenance cost greater than $ 200.00.
 ...
 SELECT 
    P.CUST_ID,
    TO_CHAR(SUM(P.PAY_AMOUNT), '$9999.99') AS 'TOTAL RENTAL PAYMENT'
FROM
    CUSTOMER C
        RIGHT JOIN
...
SELECT 
    PAY_METHOD,
    TO_CHAR(SUM(PAY_AMOUNT), '$9999.99') AS 'TOTAL PAYMENT AMOUNT'
FROM
    PAYMENT
GROUP BY PAY_METHOD
HAVING SUM(PAY_AMOUNT) > 1000;
-- Retrieves a list of payment methods with the transaction amount more than $1,000.00.
...
/* 
Retrieves customer’s first and last name (in that order) under the context of customer’s id from 
payment (joining rental, vehicle, and maintenance) from the vendor Discount Tire.
*/
